...
DESCRIPTION: 	Sends an resource intensive query based around the systemUpdate function to the GraphQL endpoint, using the introspection.json template.  This can be modified if needed.
TYPE:			Detection
*/

func ResourceIntesiveQuery(client *http.Client, Endpoint string) {
	data, _ := utils.ReadFile("templates/graphql/resourceintensivequery.json")
	...
	fmt.Println("")
	fmt.Println(color.Green + "[*] Sending the Resource Intensive Query..." + color.Reset)
