...
func CircularQuery(client *http.Client, Endpoint string) {
	data, _ := utils.ReadFile("templates/graphql/circularquery.json")
	...
	// send the request, if it fails, we DOS'd it!
	resp, err := client.Do(req)
	if err != nil {
		fmt.Println(color.Red + "[!] Something went wrong, we may have broke it!" + color.Reset)
		return
	}
	...
	fmt.Println("")
	fmt.Println(color.Green + "[*] Sending the circular query..." + color.Reset)
